...
figure;
for subjects = 1:length(subjectsIDs)
    subplot(2,1,1);plot([2,6,20],explainedRatio(subjects,:),'-','Color',[0.7,0.7,0.7]);hold on
    subplot(2,1,2);plot([2,6,20],squeeze(Explained(subjects,:,1)),'-','Color',[0.7,0.7,0.7]);hold on
end

subplot(2,1,1);plot([2,6,20],mean(explainedRatio(:,:),1),'.-','MarkerSize',30,'Color',[0 0 0]);hold on;
ylabel('\sigma_{pc2}^2/\sigma_{pc1}^2')
set(gca,'XTick',[2,6,20])
subplot(2,1,2);plot([2,6,20],nanmean(squeeze(Explained(:,:,1)),1),'.-','MarkerSize',30,'Color',[0 0 0]);hold on            
ylabel('\sigma_{pc1}^2');xlabel('sizes')
set(gca,'XTick',[2,6,20])