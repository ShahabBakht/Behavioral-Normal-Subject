...
for rmax = [0,0.09,0.18,0.27,0.36,0.45]
    ...
    [mtpopulationTarget, RTarget, COVTarget] = MTpopulation(TargetTarget,npsTarget,npdTarget,rmax);
...
for i = max(popSize)./popSize%log2(2:(62/5):64-62/5)
    ...
    [mtpopulationDots, RDots, COVDots] = MTpopulation(Target,npsDots(k),npdDots(k),rmax);
    [TargetEstimateDots] = DecodeMTpopulation(mtpopulationDots, real(RDots));
    
    estimateSPDDots(k,j,trial,:) = TargetEstimateDots.SPDest;
    estimateDIRDots(k,j,trial,:) = TargetEstimateDots.DIRest;
    ...
    R(k,j,trial) = mdl.Rsquared.Ordinary;
...
figure; plot(nanmean(bias,3),'-o','LineWidth',2)